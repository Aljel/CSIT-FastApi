    ...
    async def create(self, data: CategoryCreate) -> CategoryResponse:
        ...
        try:
            with self._database.session() as session:
                ...
                logger.info(f"Категория '{created.title}' создана (ID: {created.id})")
    ...
    async def update(self, category_id: int, data: CategoryUpdate) -> CategoryResponse:
        ...
        try:
            with self._database.session() as session:
                ...
                updated = self._repo.update(session, category, update_data)
            ...
            raise CategoryNameIsNotUniqueException(title=data.title or "")
